...
/* pd_table exportado pelo boot64.asm (global pd_table) */
extern uint64_t pd_table[];

/* ------------------------------------------------------------
 * Fonte bitmap 8x8, ASCII 32-126 (95 caracteres)
 * ------------------------------------------------------------ */
...
/* ------------------------------------------------------------
 * Mapeia o framebuffer no espaço de endereço virtual usando
 * 2 huge pages de 2MB no pd_table exportado pelo boot64.asm.
 * ------------------------------------------------------------ */
void vesa_map_framebuffer(uint64_t fb_addr)
{
    uint64_t idx = fb_addr / 0x200000;

    pd_table[idx]     = (fb_addr) | 0x83;
    pd_table[idx + 1] = (fb_addr + 0x200000) | 0x83;

    /* recarrega CR3 para invalidar o TLB (mesmo valor, força reload) */
    __asm__ volatile (
        "mov %%cr3, %%rax\n\t"
        "mov %%rax, %%cr3\n\t"
        ::: "rax", "memory"
